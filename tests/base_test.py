...
Этот класс должен быть родительским классом для каждого теста, кроме тестов на уровне юнита.
Он позволяет динамически создавать экземпляр базы данных
и гарантирует, что это будет новая, пустая база данных при каждом запуске теста.
"""
...
# !!! Это указывает SQLAlchemy, какое приложение использовать для своих операций с базой данных !!! (в app.py - app = Flask(__name__))
            db.init_app(app)
            db.create_all()
